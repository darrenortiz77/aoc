...
td-yn`;
  ...
  public solve(input?: string) {    
    ...
    bronKerbosch(new Set(), new Set(graph.values()), new Set(), cliques, null);

    // find largest clique
    let maxSize = 0;
    let maxClique: Set<Node>;
    ...
    // alphabetize computer names
    const computerNames: string[] = [];
    for (const node of maxClique!) {
      computerNames.push(node.name);
    }
    computerNames.sort();

    return {
      performance: performance.now() - performanceStart, 
      result: computerNames.join(',')
    }
  }
}
