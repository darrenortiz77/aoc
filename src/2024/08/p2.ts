    ...
    for (const [, antennaePos] of antennae) {
      for (let a=0; a < antennaePos.length-1; a++) {
        for (let b=a+1; b < antennaePos.length; b++) {
          ...
          let antinode1: Pos = [antennaePos[a][0] - deltaRow, antennaePos[a][1] - deltaCol];
          let antinode2: Pos = [antennaePos[b][0] + deltaRow, antennaePos[b][1] + deltaCol];

          antinodes.add(`${antennaePos[a].join('_')}`);
          antinodes.add(`${antennaePos[b].join('_')}`);

          while (this.isValidPos(antinode1, grid)) {
            antinodes.add(`${antinode1.join('_')}`);
            antinode1 = [antinode1[0] - deltaRow, antinode1[1] - deltaCol];
          }
          while (this.isValidPos(antinode2, grid)) {
