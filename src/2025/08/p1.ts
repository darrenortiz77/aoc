...
425,690,689`;
  ...
  public solve(input?: string) {    
    ...
    const numConnections = parsed.length > 50 ? 1000 : 10;
    ...
    for (let i=0; i < numConnections; i++) {
      ...
      if (!circuit0.has(j1) || !circuit1.has(j0)) {
        ...
        circuits.delete(circuit0);
        circuits.delete(circuit1);
        circuits.add(newCircuit);
      }
    }

    // sort the list of circuits from biggest to smallest
    const circuitsSorted = [...circuits].sort((a, b) => b.size - a.size);

    result = 1;
    for (let i=0; i < Math.min(3, circuitsSorted.length); i++) {
      result *= circuitsSorted[i].size;
    }
